...
public abstract class ModelElement extends PlatformObject implements
	...
	protected Object openWhenClosed(Object info, IProgressMonitor monitor)
		...
		try {
			...
			if (info == null) { // a source ref element could not be opened
				...
				// cannot remove below line, this method is expected to
				// throw an exception if not exists.
				// or else any ModelElement.exists() will always return true.
	...
	protected ArrayList getChildrenOfType(int type, IProgressMonitor monitor)
		...
		for (int i = 0; i < size; ++i) {
			...
			if (elt.getElementType() == type) {
				...
				list.addAll(elt.getChildrenOfType(type, monitor));
			}
	...
	protected void toStringAncestors(StringBuffer buffer) {
		...
		if (parentElement != null && parentElement.getParent() != null) {
			...
			parentElement.toStringInfo(0, buffer, NO_INFO, false); // don't
			// show
			// resolved
			// info
		...
		if (this instanceof ISourceReference) {
			...
			for (int i = children.length - 1; i >= 0; i--) {
				...
				if (aChild instanceof SourceRefElement) {
					...
					// fix for 2091 and 3174, ebay start
					if (child instanceof IJSMethod) {
						start = ((IJSMethod) child).getNameRange().getOffset();
						end = start
								+ ((IJSMethod) child).getNameRange()
										.getLength() + range.getLength();
					}
					// ebay end
					if (start <= position && position <= end) {
						...
						if (child instanceof IField
								&& child.getParent() instanceof IJSMethod) {
