 ...
 * instances. A <code>ContextFactory</code> can also notify listeners about
 ...
 * {@link #makeContext()} of the current global ContextFactory. See
 * {@link #getGlobal()} and {@link #initGlobal(ContextFactory)}.
 * <p>
 * It is also possible to use explicit ContextFactory instances for Context
 * creation. This is useful to have a set of independent Rhino runtime instances
 * under single JVM. See {@link #call(ContextAction)}.
 * <p>
 * The following example demonstrates Context customization to terminate scripts
 * running more then 10 seconds and to provide better compatibility with
 * JavaScript code using MSIE-specific features.
 * 
 * <pre>
...
public class ContextFactory {
	...
	/**
	 * Check if global factory was set. Return true to indicate that
	 * {@link #initGlobal(ContextFactory)} was already called and false to
	 * indicate that the global factory was not explicitly set.
	 * 
	 * @see #getGlobal()
	 * @see #initGlobal(ContextFactory)
	 */
	 ...
	 * thread. This is a callback method used by Rhino to create {@link Context}
	 * instance when it is necessary to associate one with the current execution
	 * thread. <tt>makeContext()</tt> is allowed to call
	...
	/**
	 * Implementation of {@link Context#hasFeature(int featureIndex)}. This can
	 ...
	 * used by the <code>Context</code> instances produced by this factory.
	 * 
	 * See {@link Context#getE4xImplementationFactory} for details.
	 */
	protected com.xored.org.mozilla.javascript.xml.XMLLib.Factory getE4xImplementationFactory() {
		...
		if (Kit.classOrNull("org.apache.xmlbeans.XmlCursor") != null) {
			return com.xored.org.mozilla.javascript.xml.XMLLib.Factory
	...
	/**
	 * Get ClassLoader to use when searching for Java classes. Unless it was
	 * explicitly initialized with
	 ...
	 * execute a script or function that will create the first stack frame with
	...
	/**
	 * Implementation of
	 * {@link Context#observeInstructionCount(int instructionCount)}. This can
	 * be used to customize {@link Context} without introducing additional
	 * subclasses.
	 */
	protected void observeInstructionCount(Context cx, int instructionCount) {
	...
	/**
	 * Call {@link ContextAction#run(Context cx)} using the {@link Context}
	 * instance associated with the current thread. If no Context is associated
	 * with the thread, then {@link #makeContext()} will be called to construct
	 * new Context instance. The instance will be temporary associated with the
