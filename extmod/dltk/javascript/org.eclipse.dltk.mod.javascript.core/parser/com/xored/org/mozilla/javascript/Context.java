...
public class Context {
	 ...
	 * <tt>toSource()</tt>. That is it returns JS source with code to create
	 * an object with all enumeratable fields of the original object instead of
	 ...
	 * <tt>hasFeature(FEATURE_PARENT_PROTO_PROPERTIES)</tt> returns true,
	 * treat <tt>__parent__</tt> and <tt>__proto__</tt> as special
	 * properties.
	 * <p>
	 ...
	 * while <tt>x.__proto__ = y</tt> changes the prototype chain of the
	 * object <tt>x</tt> to point to <tt>y</tt>,
	 ...
	 * exist is executed. When the feature is off such assignments creates new
	 ...
	 * the feature off results in Error objects that don't have additional
	 * non-ECMA properties when constructed using the ECMA-defined single-arg
	 * constructor and is thus desirable if a stricter ECMA compliance is
	 * desired, specifically adherence to the point 15.11.5. of the standard.
	 * <p>
	 * By default {@link #hasFeature(int)} returns false.
	 * 
	 * @since 1.6 Release 6
	 ...
	 * </pre>
	 * 
	 * Instead of using <tt>enter()</tt>, <tt>exit()</tt> pair consider
	 * using {@link #call(ContextAction)} which guarantees proper association of
	 * Context instances with the current thread and is faster. With this method
	 ...
	 * current thread and a Context if the prior call to <code>enter()</code>
	 * on this thread newly associated a Context with this thread. Once the
	 * current thread no longer has an associated Context, it cannot be used to
	 * execute JavaScript until it is again associated with a Context.
	 * 
	 * @see org.mozilla.javascript.Context#enter()
	 * @see #call(ContextAction)
	 * @see ContextFactory#call(ContextAction)
	 ...
	 * thisObj, Object[] args)} using the Context instance associated with the
	 ...
	 * <p>
	 * It is allowed to use null for <tt>factory</tt> argument in which case
	 * the factory associated with the scope will be used to create new context
	 * instances.
	 * 
	 * @see ContextFactory#call(ContextAction)
	 */
	...
	/**
	 * Return {@link ContextFactory} instance used to create this Context or the
	 * result of {@link ContextFactory#getGlobal()} if no factory was used for
	 * Context creation.
	 */
	 ...
	 * {@link #unseal(Object sealKey)} with the same key unseals the object. If
	 * <tt>sealKey</tt> is null, unsealing is no longer possible.
	 * 
	 * @see #isSealed()
	 * @see #unseal(Object)
	 */
	 ...
	 * language version, <i>relNum</i> is the release number, and <i>date</i>
	 * is the release date for that specific release in the form "yyyy mm dd".
	 * 
	...
	final Script compileString(String source, Interpreter compiler,
	 ...
	 * <tt>Number.doubleValue()</tt> resulting in a precision loss if the
	 * number can not fit into double.
	 * <p>
	 ...
	 * {@link WrapFactory#wrap(Context cx, Scriptable scope, Object obj, Class
	 * staticType)} as in:
	 * 
	 * <pre>
	...
	public static Object toType(Object value, Class desiredType)
		...
		try {
			...
			IllegalArgumentException ex2 = new IllegalArgumentException(ex
					.getMessage());
	 ...
	 * a subclass of {@link EvaluatorException}. The resulting exception object
	 * always contains source name and line number of script that triggered
	 * exception.
	 * <p>
	 ...
	 * this <code>Context</code>. Note that the XMLLib.Factory interface
	 ...
	 * <code>setInstructionObserverThreshold()</code>. The method is useful
	 * to observe long running scripts and if necessary to terminate them.
	 * <p>
	 ...
	 * <p>
	 * The default implementation calls
	 * {@link ContextFactory#observeInstructionCount(Context cx, int
	 * instructionCount)} that allows to customize Context behavior without
	 * introducing Context subclasses.
	 * 
	...
	private Object compileImpl(Scriptable scope, Reader sourceReader,
		...
		if (returnFunction) {
			if (!(tree.getFunctionCount() == 1 && tree.getFirstChild() != null && tree
					.getFirstChild().getType() == Token.FUNCTION)) {
		...
		Object bytecode = compiler.compile(compilerEnv, tree, encodedSource,
				returnFunction);
