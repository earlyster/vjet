...
public class Interpreter {
	...
	// Stack: ... value1 -> ... value1 value1
			...
			// To create and populate array to hold values for [] and {}
			// literals
			...
			// Load string register to prepare for the following string
			// operation
			Icode_REG_STR_C0 = -41,
			Icode_REG_STR_C1 = -42,
			Icode_REG_STR_C2 = -43,
			...
			// Version of getvar/setvar that read var index directly from
			// bytecode
	...
	// ECF_ or Expression Context Flags constants: for now only TAIL is
	// available
	...
	private static class CallFrame implements Cloneable, Serializable {
		...
		// sDbl[i]: if stack[i] is UniqueTag.DOUBLE_MARK, sDbl[i] holds the
		// number value
	...
	public Object compile(CompilerEnvirons compilerEnv, ScriptOrFnNode tree,
		...
		itsData = new InterpreterData(compilerEnv.getLanguageVersion(),
				scriptOrFn.getSourceName(), encodedSource);
	...
	private static void dumpICode(InterpreterData idata) {
		...
		if (table != null) {
			out.println("Exception handlers: " + table.length
					/ EXCEPTION_SLOT_SIZE);
	...
	static Object interpret(InterpretedFunction ifun, Context cx,
		...
		initFrame(cx, scope, thisObj, args, null, 0, args.length, ifun, null,
				frame);
		...
		StateLoop: for (;;) {
			withoutExceptions: try {
					...
					jumplessRun: {
						...
						case Icode_STARTSUB:
							if (stackTop == frame.emptyStackTop + 1) {
								// Call from Icode_GOSUB: store return PC
								...
								// Call from exception handler: exception object
								// is already stored
								// in the local
						...
						case Icode_RETSUB: {
							...
							if (value != DBL_MRK) {
								// Invocation from exception handler, restore
								// object to rethrow
								throwable = value;
								break withoutExceptions;
							}
						...
						case Token.NOT:
							stack[stackTop] = ScriptRuntime
									.wrapBoolean(!stack_boolean(frame, stackTop));
							continue Loop;
						case Token.BINDNAME:
							stack[++stackTop] = ScriptRuntime.bind(cx,
						...
						case Token.SETNAME: {
							...
							stack[stackTop] = ScriptRuntime.setName(lhs, rhs,
									cx, frame.scope, stringReg);
							continue Loop;
						}
						case Icode_SETCONST: {
							Object rhs = stack[stackTop];
							...
							--stackTop;
							Scriptable lhs = (Scriptable) stack[stackTop];
							stack[stackTop] = ScriptRuntime.setConst(lhs, rhs,
									cx, stringReg);
							continue Loop;
						}
						case Token.DELPROP: {
						...
						case Token.GETPROP: {
							...
							if (lhs == DBL_MRK)
								lhs = ScriptRuntime.wrapNumber(sDbl[stackTop]);
							stack[stackTop] = ScriptRuntime.getObjectProp(lhs,
									stringReg, cx);
							continue Loop;
						}
						case Token.SETPROP: {
							...
							if (lhs == DBL_MRK)
								lhs = ScriptRuntime.wrapNumber(sDbl[stackTop]);
							stack[stackTop] = ScriptRuntime.setObjectProp(lhs,
									stringReg, rhs, cx);
							continue Loop;
						}
						case Icode_PROP_INC_DEC: {
							...
							if (lhs == DBL_MRK)
								lhs = ScriptRuntime.wrapNumber(sDbl[stackTop]);
							stack[stackTop] = ScriptRuntime.propIncrDecr(lhs,
						...
						case Token.GETELEM: {
							...
							if (id != DBL_MRK) {
								value = ScriptRuntime
										.getObjectElem(lhs, id, cx);
							} else {
								double d = sDbl[stackTop + 1];
								value = ScriptRuntime
										.getObjectIndex(lhs, d, cx);
							}
						...
						case Token.SETELEM: {
							...
							Object id = stack[stackTop + 1];
							if (id != DBL_MRK) {
								value = ScriptRuntime.setObjectElem(lhs, id,
										rhs, cx);
							} else {
								double d = sDbl[stackTop + 1];
								value = ScriptRuntime.setObjectIndex(lhs, d,
										rhs, cx);
							}
							stack[stackTop] = value;
							continue Loop;
						...
						case Icode_ELEM_INC_DEC: {
							...
							if (lhs == DBL_MRK)
								lhs = ScriptRuntime.wrapNumber(sDbl[stackTop]);
							stack[stackTop] = ScriptRuntime.elemIncrDecr(lhs,
									rhs, cx, iCode[frame.pc]);
							++frame.pc;
							continue Loop;
						}
						case Token.GET_REF: {
						...
						case Token.SET_REF: {
							...
							--stackTop;
							Ref ref = (Ref) stack[stackTop];
							stack[stackTop] = ScriptRuntime.refSet(ref, value,
									cx);
							continue Loop;
						}
						case Token.DEL_REF: {
						...
						case Icode_REF_INC_DEC: {
							Ref ref = (Ref) stack[stackTop];
							stack[stackTop] = ScriptRuntime.refIncrDecr(ref,
									cx, iCode[frame.pc]);
							++frame.pc;
							continue Loop;
						}
						...
						case Icode_NAME_AND_THIS:
							...
							++stackTop;
							stack[stackTop] = ScriptRuntime
									.getNameFunctionAndThis(stringReg, cx,
											frame.scope);
							++stackTop;
							stack[stackTop] = ScriptRuntime
						...
						case Icode_PROP_AND_THIS: {
							...
							// stringReg: property
							stack[stackTop] = ScriptRuntime
									.getPropFunctionAndThis(obj, stringReg, cx);
							++stackTop;
							stack[stackTop] = ScriptRuntime
									.lastStoredScriptable(cx);
							continue Loop;
						...
						case Icode_ELEM_AND_THIS: {
							...
							if (obj == DBL_MRK)
								obj = ScriptRuntime
										.wrapNumber(sDbl[stackTop - 1]);
							Object id = stack[stackTop];
							if (id == DBL_MRK)
								id = ScriptRuntime.wrapNumber(sDbl[stackTop]);
							stack[stackTop - 1] = ScriptRuntime
									.getElemFunctionAndThis(obj, id, cx);
							stack[stackTop] = ScriptRuntime
									.lastStoredScriptable(cx);
							continue Loop;
						}
						case Icode_VALUE_AND_THIS: {
							...
							if (value == DBL_MRK)
								value = ScriptRuntime
										.wrapNumber(sDbl[stackTop]);
							stack[stackTop] = ScriptRuntime
									.getValueFunctionAndThis(value, cx);
							++stackTop;
							stack[stackTop] = ScriptRuntime
									.lastStoredScriptable(cx);
							continue Loop;
						...
						case Icode_CALLSPECIAL: {
							...
							if (isNew) {
								// stack change: function arg0 .. argN ->
								// newResult
								stackTop -= indexReg;
								...
								if (function == DBL_MRK)
									function = ScriptRuntime
											.wrapNumber(sDbl[stackTop]);
								Object[] outArgs = getArgsArray(stack, sDbl,
										stackTop + 1, indexReg);
								stack[stackTop] = ScriptRuntime.newSpecial(cx,
										function, outArgs, frame.scope,
										callType);
							} else {
								// stack change: function thisObj arg0 .. argN
								// -> result
								stackTop -= 1 + indexReg;
								...
								Object[] outArgs = getArgsArray(stack, sDbl,
										stackTop + 2, indexReg);
								stack[stackTop] = ScriptRuntime.callSpecial(cx,
										function, functionThis, outArgs,
						...
						case Token.REF_CALL: {
							...
							// are already Scriptable and Callable objects
							// respectively
							...
							if (op == Token.REF_CALL) {
								Object[] outArgs = getArgsArray(stack, sDbl,
										stackTop + 2, indexReg);
								stack[stackTop] = ScriptRuntime.callRef(fun,
							...
							if (fun instanceof InterpretedFunction) {
								...
								if (frame.fnOrScript.securityDomain == ifun.securityDomain) {
									...
									if (op == Icode_TAIL_CALL) {
										// In principle tail call can re-use the
										// current
										// frame and its stack arrays but it is
										// hard to
										// do properly. Any exceptions that can
										// legally
										// happen during frame re-initialization
										// including
										// StackOverflowException during
										// innocent looking
										// System.arraycopy may leave the
										// current frame
										// data corrupted leading to undefined
										// behaviour
										// in the catch code bellow that unwinds
										// JS stack
										// on exceptions. Then there is issue
										// about frame release
										// end exceptions there.
										// To avoid frame allocation a released
										// frame
										// can be cached for re-use which would
										// also benefit
										// non-tail calls but it is not clear
										// that this caching
										// would gain in performance due to
										// potentially
										// bad iteraction with GC.
										callParentFrame = frame.parentFrame;
										// Release the current frame. See Bug
										// #344501 to see why
										// it is being done here.
										// TODO: If using the graphical
										// debugger, tail call
										// optimization will create a "hole" in
										// the context stack.
										// The correct thing to do may be to
										// disable tail call
										// optimization if the code is being
										// debugged.
										exitFrame(cx, frame, null);
									}
									initFrame(cx, calleeScope, funThisObj,
											stack, sDbl, stackTop + 2,
											indexReg, ifun, callParentFrame,
											calleeFrame);
									if (op != Icode_TAIL_CALL) {
							...
							if (fun instanceof Continuation) {
								...
								cjump = new ContinuationJump(
										(Continuation) fun, frame);

								// continuation result is the first argument if
								// any
								// of contination call
								if (indexReg == 0) {
							...
							if (fun instanceof IdFunctionObject) {
								...
								if (Continuation
										.isContinuationConstructor(ifun)) {
							...
							Object[] outArgs = getArgsArray(stack, sDbl,
									stackTop + 2, indexReg);
							stack[stackTop] = fun.call(cx, calleeScope,
									funThisObj, outArgs);
						...
						case Token.NEW: {
							...
							if (lhs instanceof InterpretedFunction) {
								...
								if (frame.fnOrScript.securityDomain == f.securityDomain) {
									Scriptable newInstance = f.createObject(cx,
											frame.scope);
									CallFrame calleeFrame = new CallFrame();
									initFrame(cx, frame.scope, newInstance,
											stack, sDbl, stackTop + 1,
											indexReg, f, frame, calleeFrame);
							...
							Object[] outArgs = getArgsArray(stack, sDbl,
									stackTop + 1, indexReg);
							stack[stackTop] = fun.construct(cx, frame.scope,
									outArgs);
							continue Loop;
						}
						case Token.TYPEOF: {
						...
						case Icode_TYPEOFNAME:
							stack[++stackTop] = ScriptRuntime.typeofName(
									frame.scope, stringReg);
							continue Loop;
						case Token.STRING:
						...
						case Token.NUMBER:
							...
							continue Loop;
						case Token.NAME:
							stack[++stackTop] = ScriptRuntime.name(cx,
									frame.scope, stringReg);
							continue Loop;
						case Icode_NAME_INC_DEC:
							stack[++stackTop] = ScriptRuntime
									.nameIncrDecr(frame.scope, stringReg, cx,
											iCode[frame.pc]);
							++frame.pc;
							continue Loop;
						case Icode_SETCONSTVAR1:
						...
						case Token.GETVAR:
							...
							if (!frame.useActivation) {
								...
								stringReg = frame.idata.argNames[indexReg];
								stack[stackTop] = frame.scope.get(stringReg,
										frame.scope);
							}
							continue Loop;
						case Icode_VAR_INC_DEC: {
							...
							if (!frame.useActivation) {
								...
								double d2 = ((incrDecrMask & Node.DECR_FLAG) == 0) ? d + 1.0
										: d - 1.0;
								varDbls[indexReg] = d2;
								sDbl[stackTop] = ((incrDecrMask & Node.POST_FLAG) == 0) ? d2
										: d;
							} else {
								String varName = frame.idata.argNames[indexReg];
								stack[stackTop] = ScriptRuntime.nameIncrDecr(
										frame.scope, varName, cx, incrDecrMask);
							}
							++frame.pc;
							continue Loop;
						}
						...
						case Token.ENTERWITH: {
							...
							if (lhs == DBL_MRK)
								lhs = ScriptRuntime.wrapNumber(sDbl[stackTop]);
							--stackTop;
							frame.scope = ScriptRuntime.enterWith(lhs, cx,
									frame.scope);
							continue Loop;
						}
						case Token.LEAVEWITH:
						...
						case Token.CATCH_SCOPE: {
							...
							stack[indexReg] = ScriptRuntime.newCatchScope(
									caughtException, lastCatchScope, stringReg,
									cx, frame.scope);
							++frame.pc;
							continue Loop;
						}
						...
						case Token.ENUM_INIT_VALUES: {
							...
							--stackTop;
							indexReg += frame.localShift;
							stack[indexReg] = ScriptRuntime.enumInit(lhs, cx,
						...
						case Token.ENUM_ID: {
							...
							++stackTop;
							stack[stackTop] = (op == Token.ENUM_NEXT) ? (Object) ScriptRuntime
									.enumNext(val)
						...
						case Token.REF_SPECIAL: {
							...
							if (obj == DBL_MRK)
								obj = ScriptRuntime.wrapNumber(sDbl[stackTop]);
							stack[stackTop] = ScriptRuntime.specialRef(obj,
									stringReg, cx);
							continue Loop;
						}
						case Token.REF_MEMBER: {
							...
							if (obj == DBL_MRK)
								obj = ScriptRuntime.wrapNumber(sDbl[stackTop]);
							stack[stackTop] = ScriptRuntime.memberRef(obj,
									elem, cx, indexReg);
							continue Loop;
						}
						case Token.REF_NS_MEMBER: {
							...
							if (obj == DBL_MRK)
								obj = ScriptRuntime.wrapNumber(sDbl[stackTop]);
							stack[stackTop] = ScriptRuntime.memberRef(obj, ns,
									elem, cx, indexReg);
							continue Loop;
						}
						case Token.REF_NAME: {
							...
							stack[stackTop] = ScriptRuntime.nameRef(name, cx,
						...
						case Token.REF_NS_NAME: {
							...
							if (ns == DBL_MRK)
								ns = ScriptRuntime.wrapNumber(sDbl[stackTop]);
							stack[stackTop] = ScriptRuntime.nameRef(ns, name,
									cx, frame.scope, indexReg);
							continue Loop;
						}
						case Icode_SCOPE_LOAD:
							indexReg += frame.localShift;
						...
						case Icode_SCOPE_SAVE:
							...
							continue Loop;
						case Icode_CLOSURE_EXPR:
							stack[++stackTop] = InterpretedFunction
									.createFunction(cx, frame.scope,
											frame.fnOrScript, indexReg);
							continue Loop;
						case Icode_CLOSURE_STMT:
							initFunction(cx, frame.scope, frame.fnOrScript,
									indexReg);
							continue Loop;
						case Token.REGEXP:
						...
						case Token.OBJECTLIT: {
							...
							if (op == Token.OBJECTLIT) {
								...
								val = ScriptRuntime.newArrayLiteral(data,
										skipIndexces, cx, frame.scope);
							}
							stack[stackTop] = val;
							continue Loop;
						}
						case Icode_ENTERDQ: {
							...
							if (lhs == DBL_MRK)
								lhs = ScriptRuntime.wrapNumber(sDbl[stackTop]);
							--stackTop;
							frame.scope = ScriptRuntime.enterDotQuery(lhs,
									frame.scope);
							continue Loop;
						}
						case Icode_LEAVEDQ: {
							boolean valBln = stack_boolean(frame, stackTop);
							Object x = ScriptRuntime.updateDotQuery(valBln,
									frame.scope);
							if (x != null) {
								stack[stackTop] = x;
				...
				if (frame.parentFrame != null) {
					...
					setCallResult(frame, interpreterResult,
	...
	private static void initFrame(Context cx, Scriptable callerScope,
		...
		frame.frameIndex = (parentFrame == null) ? 0
				: parentFrame.frameIndex + 1;
				...
				for (;;) {
					if (scope instanceof NativeCall) {
						...
						if (scope == null
							...
							Kit.codeBug();
							break; // Never reached, but keeps the static
									// analyzer happy about "scope" not being
									// null 5 lines above.
						}
					}
				}
	...
	private static void captureContinuation(Context cx, CallFrame frame,
		...
		ScriptRuntime.setObjectProtoAndParent(c, ScriptRuntime
				.getTopCallScope(cx));
