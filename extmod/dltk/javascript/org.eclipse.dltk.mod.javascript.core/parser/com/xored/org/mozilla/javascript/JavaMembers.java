...
public class JavaMembers {
	...
	void put(Scriptable scope, String name, Object javaObject, Object value,
		...
		if (member instanceof BeanProperty) {
			...
			if (bp.setters == null || value == null) {
				...
				Object[] args = { value };
				bp.setters.call(Context.getContext(), ScriptableObject
			...
			try {
				...
				throw Context.reportRuntimeError3(
						"msg.java.internal.field.type", value.getClass()
								.getName(), field, javaObject.getClass()
								.getName());
			}
		}
	}
	...
	private Object getExplicitFunction(Scriptable scope, String name,
		...
		if (methodOrCtor != null) {
			...
			if (methodOrCtor.isCtor()) {
				NativeJavaConstructor fun = new NativeJavaConstructor(
						methodOrCtor);
	...
	/**
	 * Retrieves mapping of methods to accessible methods for a class. In case
	 * the class is not public, retrieves methods with same signature as its
	...
	private static void discoverAccessibleMethods(Class clazz, Map map,
		...
		if (Modifier.isPublic(clazz.getModifiers())) {
			try {
				...
				Context
						.reportWarning("Could not discover accessible methods of class "
								+ clazz.getName()
	...
	/**
	 * @return the fieldAndMethods
	 */
	public Hashtable getFieldAndMethods(boolean isStatic) {
		return isStatic ? staticFieldAndMethods : fieldAndMethods;
	}
...
class FieldAndMethods extends NativeJavaMethod {
	...
	public Object getDefaultValue(Class hint) {
		...
		try {
			...
			throw Context.reportRuntimeError1("msg.java.internal.private",
					field.getName());
		}
		Context cx = Context.getContext();
