...
public class Parser {
	...
	protected String encodedSource;
	...
	private ScriptOrFnNode parse() throws IOException {
		...
		if (this.syntaxErrorCount != 0) {
			...
			errorReporter.runtimeError(msg, sourceURI, baseLineno, null, 0);
		}
	...
	private Node function(int functionType) throws IOException, ParserException {
		...
		if (memberExprNode != null) {
			pn = nf.createAssignment(Token.ASSIGN, memberExprNode, pn, ts
					.getCursor());
			if (functionType != FunctionNode.FUNCTION_EXPRESSION) {
	...
	private Node statementHelper(Node statementLabel) throws IOException,
		...
		case Token.IF: {
			...
			if (ifTrue.hasChildren()) {
				...
				ifTrue.setPosition(ts.getCursor());
			Node ifFalse = null;
			if (matchToken(Token.ELSE)) {
				...
				if (ifFalse.hasChildren()) {
					...
					ifFalse.setPosition(ts.getCursor());
			}
			...
			if (peek == Token.CATCH) {
				while (matchToken(Token.CATCH)) {
					...
					nf.addChildToBack(catchblocks, nf.createCatch(varName,
							catchCond, statements(), ts.getLineno(), ts
									.getCursor()));
	 ...
	 * 
	 * @param context
	 *            A token value: either VAR, CONST or FOR depending on context.
		...
		for (;;) {
			...
			if (context == Token.CONST) {
				if (!currentScriptOrFn.addConst(s)) {
					...
					// what it is.
					if (currentScriptOrFn
							.addVar(s, nameStart, nameEnd, comment) != ScriptOrFnNode.DUPLICATE_CONST)
						addError("msg.var.redecl", s);
					else
						addError("msg.const.redecl", s);
				}
			} else {
				int dupState = currentScriptOrFn.addVar(s, nameStart, nameEnd,
						comment);
	...
	private Node xmlInitializer() throws IOException {
		...
		if (cb != null)
			cb.xmlTokenStart(offset, xml, ts.getCursor());
