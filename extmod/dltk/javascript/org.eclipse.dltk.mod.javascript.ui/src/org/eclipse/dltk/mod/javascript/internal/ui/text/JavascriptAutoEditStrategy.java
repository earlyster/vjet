...
import org.eclipse.dltk.mod.compiler.InvalidInputException;
...
import org.eclipse.dltk.mod.javascript.scriptdoc.ITerminalSymbols;
...
public class JavascriptAutoEditStrategy extends
	...
	private static void installJavaStuff(Document document) {
		...
		FastPartitioner partitioner = new FastPartitioner(
			...
			public void connect(IDocument doc, boolean delayInitialization) {

				super.connect(doc, delayInitialization);
				// printPartitions(doc);//for debug use
			}

			// public void printPartitions(IDocument document) {
			// StringBuffer buffer = new StringBuffer();
			//
			// ITypedRegion[] partitions = computePartitioning(0, document
			// .getLength());
