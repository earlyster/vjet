...
public class RemoveBlockCommentAction extends BlockCommentAction {
	...
	/*
	 * @see org.eclipse.jdt.internal.ui.actions.AddBlockCommentAction#runInternal(org.eclipse.jface.text.ITextSelection,
	...
	protected void runInternal(ITextSelection selection,
		...
		while (partEndOffset < endOffset) {
			...
			if (partition.getType() == IJavaScriptPartitions.JS_SINGLE_COMMENT
					|| partition.getType() == IJavaScriptPartitions.JS_MULTI_COMMENT) {
			...
			partition = docExtension
					.getPartition(IJavaScriptPartitions.JS_PARTITIONING,
							partEndOffset, false);
