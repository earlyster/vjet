...
public class CommandLine {
	...
	**/
	...
	private static HtmlSaxParser theParser = null;
	...
	private static void process(String src, OutputStream os)
		...
		if (hasOption(options, "--reuse")) {
			if (theParser == null) theParser = new HtmlSaxParser();
			r = theParser;
			}
		else {
			r = new HtmlSaxParser();
		...
		r.setProperty(HtmlSaxParser.schemaProperty, theSchema);

		if (hasOption(options, "--nocdata")) {
			r.setFeature(HtmlSaxParser.CDATAElementsFeature, false);
			}

		if (hasOption(options, "--nons") || hasOption(options, "--html")) {
			r.setFeature(HtmlSaxParser.namespacesFeature, false);
			}

		if (hasOption(options, "--nobogons")) {
			r.setFeature(HtmlSaxParser.ignoreBogonsFeature, true);
			}

		if (hasOption(options, "--any")) {
			r.setFeature(HtmlSaxParser.bogonsEmptyFeature, false);
			}
		else if (hasOption(options, "--emptybogons")) {
			r.setFeature(HtmlSaxParser.bogonsEmptyFeature, true);
			}

		if (hasOption(options, "--norootbogons")) {
			r.setFeature(HtmlSaxParser.rootBogonsFeature, false);
			}

		if (hasOption(options, "--nodefaults")) {
			r.setFeature(HtmlSaxParser.defaultAttributesFeature, false);
			}
		if (hasOption(options, "--nocolons")) {
			r.setFeature(HtmlSaxParser.translateColonsFeature, true);
			}

		if (hasOption(options, "--norestart")) {
			r.setFeature(HtmlSaxParser.restartElementsFeature, false);
			}

		if (hasOption(options, "--ignorable")) {
			r.setFeature(HtmlSaxParser.ignorableWhitespaceFeature, true);
			}

		if (hasOption(options, "--pyxin")) {
			r.setProperty(HtmlSaxParser.scannerProperty, new PYXScanner());
			}
		...
		if (hasOption(options, "--lexical") && h instanceof LexicalHandler) {
			r.setProperty(HtmlSaxParser.lexicalHandlerProperty, h);
			}
