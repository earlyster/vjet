...
**/
public class HtmlSaxParser extends DefaultHandler implements IScanHandler, XMLReader, LexicalHandler {
	...
	private List<String> m_wellformRequied;
...
"http://xml.org/sax/features/unicode-normalization-checking";
	...
	**/
	...
	/**
	 * Specifies wellForm check is on
	 */
	public final static String wellformRequired = "wellformRequired";
	...
	private HashMap<String, Boolean> theFeatures = new HashMap<String, Boolean>();
	{
		...
		theFeatures.put(wellformRequired, Boolean.TRUE);		
		}
	...
	throws SAXNotRecognizedException, SAXNotSupportedException {
		if (name.equals(lexicalHandlerProperty)) {
			...
			if (value instanceof IAutoDetector) {
...
//		theLexicalHandler = (handler == null) ? this : handler;
//		}
//
...
//		}
	...
	public void parse(InputSource input, final List<String> wellFormedTags) throws IOException, SAXException {
		setup(wellFormedTags);
	...
	private void setup(final List<String> wellformedTags) throws SAXNotRecognizedException, SAXNotSupportedException {
		
		if (theSchema == null) theSchema = new HtmlSchema();
		if (theScanner == null) {
			theScanner = new HtmlScanner();
		} 
		if (getFeature(wellformRequired)) {
			m_wellformRequied = wellformedTags;
		}
		if (theAutoDetector == null) {
			theAutoDetector = new IAutoDetector() {
...
//		r = new BufferedReader(r);
	...
	public void adup(char[] buff, int offset, int length) throws SAXException {
		...
		//fix bug
		//INPUT:<image @/src=//test.com/logo.gif />
		//OUTPUT:<image _="_" src="//test.com/logo.gif"></image>
		if (!theAttributeName.equals("_")) {
...
//		System.err.println("%% Attribute value [" + value + "]");
		...
		//04-14-2009
		//DsfQ109_NS
		if (theAttributeName.equals("xmlns") 
				|| theAttributeName.startsWith("xmlns:")) 
		{
			theNewElement.type().setNamespace(value);	
		} else {
...
//		System.err.println("%% Got end of " + name);
		...
		if (sp == null) { 
			//V4 parser enhancement
			if (name != null && m_wellformRequied !=null && m_wellformRequied.indexOf(name) != -1){
				throw new SAXException("not wellformed element " + name);
			}
