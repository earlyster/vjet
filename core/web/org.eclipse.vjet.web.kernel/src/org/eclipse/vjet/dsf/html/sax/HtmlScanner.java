...
public class HtmlScanner implements IScanner, Locator {
	...
	*/
	...
	public void scan(Reader r0, IScanHandler h) throws IOException, SAXException {
		theState = S_PCDATA;
		PushbackReader r;
...
Integer.toString(theState));
				...
				h.aval(theOutputBuffer, 0, theSize);
				theSize = 0;
				//fix bugs: script/style w self-closing tag, while attr w/o quote
				//>>>INPUT:   <script src=a/><script src=z/>m<br/><span>s</span>
				//>>>OUTPUT:  <script src="a/"><script src=z/>m<br/><span>s</span></script>
				if (prevChar=='/' && ch=='>' && firstChar == '<'){
					theState = S_EMPTYTAG;					
...
//				System.err.println("%%% Empty tag seen");
			...
			//fix bug: script/style, and w self-closing tag, while attr with quote 
			//>>>INPUT:      <script id="a"/><script id="z"/>
			//>>>OUTPUT:     <script id="a"></script>&lt;script id="z"/&gt;
			//theState == S_EMPTYTAG && theNextState == S_CDATA, reset to the init state			
			theState = (theState != S_EMPTYTAG || theNextState != S_CDATA)
					?theNextState:S_PCDATA;		
			//help to identify empty  tag
			prevChar = ch;
		}
	...
	*/
	...
	private void save(int ch, IScanHandler h) throws IOException, SAXException {
