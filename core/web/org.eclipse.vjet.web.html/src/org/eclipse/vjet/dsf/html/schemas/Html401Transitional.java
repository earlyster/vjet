...
public class Html401Transitional {
	...
	private static void addBaseFont(final Map<HtmlTypeEnum,IElementInfo> map) {
		...
		final ElementInfo elementInfo = new ElementInfo(
			HtmlTypeEnum.BASEFONT, attrs, IContentModel.EMPTY, true, false);
		map.put(elementInfo.getType(), elementInfo);	
	}
	...
	private static void addDirAndMenu(final Map<HtmlTypeEnum,IElementInfo> map) {
		...
		final ElementInfo dir = new ElementInfo(
			HtmlTypeEnum.DIR,
			attrs,
			new ContentModelSets(includeElements,SET_BLOCK));
		map.put(dir.getType(), dir);
		final ElementInfo menu = new ElementInfo(
			HtmlTypeEnum.MENU,
	...
	/**
	 * <!ELEMENT FRAME - - (%flow;)* -- inline subwindow --> <!ATTLIST FRAME
	 * %coreattrs; -- id, class, style, title -- longdesc %URI; #IMPLIED -- link
	 * to long description (complements title) -- name CDATA #IMPLIED -- name of
	 * frame for targetting -- src %URI; #IMPLIED -- source of frame content --
	 * frameborder (1|0) 1 -- request frame borders? -- marginwidth %Pixels;
	 * #IMPLIED -- margin widths in pixels -- marginheight %Pixels; #IMPLIED --
	 * margin height in pixels -- scrolling (yes|no|auto) auto -- scrollbar or
	 * none -- align %IAlign; #IMPLIED -- vertical or horizontal alignment --
	 * height %Length; #IMPLIED -- frame height -- width %Length; #IMPLIED --
	 * frame width -- >
	 */
	private static void addFrame(final Map<HtmlTypeEnum, IElementInfo> map) {
		final IAttributeInfoMap attrs = new AttributeInfoMapImpl();
		add(attrs, ATTRS_CORE);
		...
		attrs.put(new AttributeInfoEnumeration("scrolling",
				ScrollingEnum.class, AttributeDefault.IMPLIED,
				ScrollingEnum.AUTO));
		attrs.put(IALIGN_ATTR_INFO);
		attrs.put(new AttributeInfoImpl("height"));
		attrs.put(new AttributeInfoImpl("width"));
		final ElementInfo elementInfo = new ElementInfo(HtmlTypeEnum.FRAME,
				attrs, new ContentModelSets(SET_FLOW));
		map.put(elementInfo.getType(), elementInfo);
	}
	 ...
	 *
	 */
	private static void addFrameSet(final Map<HtmlTypeEnum,IElementInfo> map) {
		final IAttributeInfoMap attrs = new AttributeInfoMapImpl();
		add(attrs,ATTRS_CORE);
		attrs.put(new AttributeInfoImpl("longdesc"));		
		attrs.put(new AttributeInfoImpl("name"));		
		attrs.put(new AttributeInfoImpl("rows"));
		attrs.put(new AttributeInfoImpl("cols"));	
		final ElementInfo elementInfo = new ElementInfo(
			HtmlTypeEnum.FRAMESET,
			attrs,
			new ContentModelSets(SET_FLOW));
		map.put(elementInfo.getType(), elementInfo);
	}
