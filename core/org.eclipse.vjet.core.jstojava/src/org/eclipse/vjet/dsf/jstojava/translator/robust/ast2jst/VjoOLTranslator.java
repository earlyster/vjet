...
public class VjoOLTranslator extends
	...
	protected IJstNode doTranslate(ObjectLiteral astObjectLiteral) {
		...
		if (astObjectLiteral.fields != null) {
			result = processPropsAndMethods(astObjectLiteral);
		} else {
			result = new ObjLiteral();
			result.setSource(TranslateHelper.getSource(astObjectLiteral,m_ctx.getSourceUtil()));
		}
	...
	private ObjLiteral processPropsAndMethods(ObjectLiteral astObjectLiteral) {
		...
		for (int i = 0; i < len; i++) {
			...
			VjoOLFieldTranslator translator = new VjoOLFieldTranslator(m_ctx);
...
//				|| scopeId == ScopeIds.DEFS) {
		...
		if (problem != null) {
			...
			if (prefix.length() > 0 && isJavaIdentifier(prefix)) {
				completion = new JstFieldOrMethodCompletion(m_ctx
						.getCurrentType(), isStaticBlock());
		...
		if (!isAfterSource && completion == null) {
			...
			if (isInvalidValidToken(token)) {
				return null;
			}
			completion = new JstFieldOrMethodCompletion(m_ctx.getCurrentType(),
					isStaticBlock());
	...
	private boolean isInvalidValidToken(String token) {
		return token == null || 
		token.indexOf("(") >= 0 ||
		token.indexOf("<") >= 0 || 	// is token a comment
		token.indexOf(">") >= 0;	// is token a comment
	}
