...
public class DefaultErrorReporter implements ErrorReporter {
	...
	public void report(ReportLevel level, String message, String resource, int startOffset, int endOffset) {
		if (level == ReportLevel.IGNORE) {
			return;
		}
		if (level == ReportLevel.WARNING) {
			warning(message, resource, startOffset, endOffset);
		} else if (level == ReportLevel.ERROR) {
			error(message, resource, startOffset, endOffset);
		}
		
	}

	public void error(String message, String resource, int startOffset, int endOffset) {
		...
		args.add(STARTOFFSET, String.valueOf(startOffset));
		args.add(ENDOFFSET, String.valueOf(endOffset));
		addError(new ErrorObject(s_errorId, ErrorSeverity.ERROR, args));
	}


	public void warning(String message, String resource, int startOffset, int endOffset) {
