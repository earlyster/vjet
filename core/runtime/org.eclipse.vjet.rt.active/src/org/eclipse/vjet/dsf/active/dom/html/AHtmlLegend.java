...
public class AHtmlLegend extends AHtmlElement implements HtmlLegend {
	
	private static final long serialVersionUID = 1L;
	
	protected AHtmlLegend(AHtmlDocument doc, DLegend legend) {
		super(doc, legend);
		populateScriptable(AHtmlLegend.class, doc == null ? BrowserType.IE_6P : doc.getBrowserType());
	}

	public String getAccessKey() {
		return getDLegend().getHtmlAccessKey();
	}

	public String getAlign() {
		return getDLegend().getHtmlAlign();
	}
	...
	public void setAccessKey(String accessKey) {
		getDLegend().setHtmlAccessKey(accessKey);
		onAttrChange(EHtmlAttr.accesskey, accessKey);
	}

	public void setAlign(String align) {
		getDLegend().setHtmlAlign(align);
	...
	private DLegend getDLegend() {
		return (DLegend) getDNode();
	}
