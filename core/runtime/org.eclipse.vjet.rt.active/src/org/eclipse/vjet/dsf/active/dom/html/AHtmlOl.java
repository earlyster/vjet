...
public class AHtmlOl extends AHtmlElement implements HtmlOl {

	private static final long serialVersionUID = 1L;
	
	protected AHtmlOl(AHtmlDocument doc, DOl node) {
		super(doc, node);
		populateScriptable(AHtmlOl.class, doc == null ? BrowserType.IE_6P : doc.getBrowserType());
	}
	...
	public int getStart() {
		return getDOl().getHtmlStart();
	}

	public String getType() {
		return getDOl().getHtmlType();
	}
	...
	public void setStart(int start) {
		getDOl().setHtmlStart(start);
		onAttrChange(EHtmlAttr.start, String.valueOf(start));
	}

	public void setType(String type) {
		getDOl().setHtmlType(type);
