...
public class AHtmlLi extends AHtmlElement implements HtmlLi {

	private static final long serialVersionUID = 1L;
	
	protected AHtmlLi(AHtmlDocument doc, DLi node) {
		super(doc, node);
		populateScriptable(AHtmlLi.class, doc == null ? BrowserType.IE_6P : doc.getBrowserType());
	}

	public String getType() {
		return getDLi().getHtmlType();
	}

	public int getValue() {
		return getDLi().getHtmlValue();
	}

	public void setType(String type) {
		getDLi().setHtmlType(type);
		onAttrChange(EHtmlAttr.type, type);
	}

	public void setValue(int value) {
		getDLi().setHtmlValue(value);
		onValueChange(String.valueOf(value));
	}
