...
import org.eclipse.vjet.dsf.html.dom.DSource;
import org.eclipse.vjet.dsf.html.dom.EHtmlAttr;
import org.eclipse.vjet.dsf.jsnative.HtmlSource;
import org.eclipse.vjet.dsf.jsnative.anno.BrowserType;

public class AHtmlSource extends AHtmlElement implements HtmlSource {
	...
	// Constructor(s)
	//
	protected AHtmlSource(AHtmlDocument doc, DSource source) {
		super(doc, source);
		populateScriptable(AHtmlSource.class, doc == null ? BrowserType.IE_6P : doc.getBrowserType());
	}
	...
	public String getSrc() {
		return getDSource().getHtmlSrc();
	}
	public void setSrc(String src) {
		getDSource().setHtmlSrc(src);
		onAttrChange(EHtmlAttr.src, src);
	}
	
	public String getType() {
		return getDSource().getHtmlType();
	}
	public void setType(String type) {
		getDSource().setHtmlType(type);
		onAttrChange(EHtmlAttr.type, type);
	}
	
	public String getMedia() {
		return getDSource().getHtmlMedia();
	}
	public void setMedia(String media) {
		getDSource().setHtmlMedia(media);
	...
	// Private
	//
	private DSource getDSource() {
		return (DSource) getDNode();
	}
}
