...
import org.eclipse.vjet.dsf.html.dom.DHr;
import org.eclipse.vjet.dsf.html.dom.EHtmlAttr;
import org.eclipse.vjet.dsf.jsnative.HtmlHr;
import org.eclipse.vjet.dsf.jsnative.anno.BrowserType;

public class AHtmlHr extends AHtmlElement implements HtmlHr {

	private static final long serialVersionUID = 1L;
	
	protected AHtmlHr(AHtmlDocument doc, DHr node) {
		super(doc, node);
		populateScriptable(AHtmlHr.class, doc == null ? BrowserType.IE_6P : doc.getBrowserType());
	}

	public String getAlign() {
		return getDHr().getHtmlAlign();
	}

	public boolean getNoShade() {
		return AHtmlHelper.booleanValueOf(EHtmlAttr.noshade,getHtmlAttribute(EHtmlAttr.noshade));
	}

	public String getSize() {
		return getDHr().getHtmlSize();
	}

	public String getWidth() {
		return getDHr().getHtmlWidth();
	}

	public void setAlign(String align) {
		getDHr().setHtmlAlign(align);
		onAttrChange(EHtmlAttr.align, align);
	}

	public void setNoShade(boolean noShade) {
		setHtmlAttribute(EHtmlAttr.noshade, noShade);
		onAttrChange(EHtmlAttr.noshade, noShade);
	}

	public void setSize(String size) {
		getDHr().setHtmlSize(size);
		onAttrChange(EHtmlAttr.size, size);
	}

	public void setWidth(String width) {
		getDHr().setHtmlWidth(width);
