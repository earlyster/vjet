...
public class BeanSerializer extends AbstractSerializer {
	...
	public boolean canSerialize(Class clazz, Class jsonClazz) {
		return (
			!clazz.isArray()
				&& !clazz.isPrimitive()
				&& !clazz.isInterface()
				&& !clazz.isEnum() // leave it to EnumSerializer
				&& !ISerializableForVjo.class.isAssignableFrom(clazz)//leave it to VjoObjSerializer
	...
	private static BeanData createBeanData(Class clazz)
		...
		PropertyDescriptor[] props = Introspector.getBeanInfo(clazz, Object.class).
	...
	public static BeanData getBeanData(Class clazz)
		...
		synchronized (BeanSerializer.class) {
			...
			WeakHashMap<Class, BeanData> newCache = new WeakHashMap<Class, BeanData>(s_beanDataCache.size()+1);
	...
	public ObjectMatch tryUnmarshall(
		...
		Iterator<Map.Entry<String, Method>> i = bd.m_writableProps.entrySet().iterator();
		while (i.hasNext()) {
		...
		while (keyIter.hasNext()) {
			...
			if (setMethod != null) {
				try {
					...
					throw new SerializationException(
						"bean " + clazz.getName() + " " + e.getMessage());
				}
			} else {
				mismatch++;
			}
	...
	public Object unmarshall(SerializerState state, Class clazz, Object o)
		...
		try {
			...
			ServiceEngineError error = new ServiceEngineError(
					IServiceEngineErrorConstants.INVALID_REQUEST_ERROR,
					IServiceEngineErrorConstants.SEVERITY, msg);
			ex.setError(error);
			throw ex;
		...
		while (i.hasNext()) {
			...
			if (setMethod != null) {
				try {
					...
					fieldVal =
						getOwner().unmarshall(state, param[0], genericParam[0], jso.get(field));
				} catch (SerializationException e) {
					String msg = "bean " + clazz.getName() + " " + e.getMessage();
					SerializationException ex = new SerializationException(msg);
					ServiceEngineError error = new ServiceEngineError(
							IServiceEngineErrorConstants.INVALID_PARAMETER_TYPE_ERROR,
				...
				if (!validator.isParamOk(field)) {
					String msg = "bean " + clazz.getName() + " does not have property " + field;
					SerializationException ex = new SerializationException(msg);
					ServiceEngineError error = new ServiceEngineError(
							IServiceEngineErrorConstants.INVALID_PARAMETER_ERROR,
							msg, IServiceEngineErrorConstants.SEVERITY);
					ex.setError(error);
					throw ex;
				}
					...
					if (null != getMethod) {
						try {
							...
							if (List.class.isAssignableFrom(returnClass)) {
								fieldVal =
									getOwner().unmarshall(state, returnClass, actualType[0], jso.get(field));
								try {
		...
		if (validator.isRequiredParamMissing(params)) {
			String msg = "bean " + clazz.getName() + " is missing a required param";
			SerializationException ex = new SerializationException(msg);
			ServiceEngineError error = new ServiceEngineError(
	...
	public Object marshall(SerializerState state, Object o)
		...
		try {
			beanState =
				(BeanSerializerState) state.get(BeanSerializerState.class);
		} catch (Exception e) {
		...
		if(isMarshallClassHints() || SerializationCtx.ctx().isForceClassHintsInBeanMashalling()) {
