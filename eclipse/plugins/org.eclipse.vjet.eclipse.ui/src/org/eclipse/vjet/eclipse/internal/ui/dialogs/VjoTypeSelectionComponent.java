...
import org.eclipse.dltk.mod.internal.ui.util.TypeNameMatchLabelProvider;
...
/**
 * This component contains all ui elements for open type dialog functionality.
 * Copy more part of functionality from {@link TypeSelectionComponent}.
 * 
 * 
 * 
 */
public class VjoTypeSelectionComponent extends Composite implements
	...
	private class ToggleStatusLineAction extends Action {
		...
		public void run() {
			...
			VjoTypeSelectionComponent.this.layout();
		}
	}
	...
	public VjoTypeSelectionComponent(Composite parent, int style,
			String message, boolean multi, IDLTKSearchScope scope,
			int elementKind, String initialFilter, ITitleLabel titleLabel,
	...
	private void createContent(final String message, int elementKind) {
		...
		fViewer = new VjoTypeInfoViewer(this, fMultipleSelection ? SWT.MULTI
				...
				fTypeSelectionExtension != null ? fTypeSelectionExtension
						.getImageProvider() : null, this.fToolkit);
		...
		table.getAccessible().addAccessibleListener(new AccessibleAdapter() {
			public void getName(AccessibleEvent e) {
				if (table.getSelectionCount() == 0) {
					e.result = Strings
							.removeMnemonicIndicator(DLTKUIMessages.TypeSelectionComponent_label);
				}
			}
		});
		fViewer.setFullyQualifyDuplicates(fSettings
				.getBoolean(FULLY_QUALIFY_DUPLICATES), false);
		...
		// Add by Oliver. Remove the filter '*' when we open the dialog at first
		// time. Otherwise if there are a large number of js file in the
		// workspace, it costs a long time to show all types.

		// if (fInitialFilterText == null) {
		// fViewer.setSearchPattern("*");
		// }
	}
	...
	private void fillViewMenu(IMenuManager viewMenu) {
		if (!fMultipleSelection) {
			...
			showStatusLineAction.setChecked(fSettings
					.getBoolean(SHOW_STATUS_LINE));
			viewMenu.add(showStatusLineAction);
		}

		if (fScope == null) {
			// When fitler the type by selected workingset, the action is registered here.
			...
			if (ws == null || (ws.isAggregateWorkingSet() && ws.isEmpty())) {
				fScope = SearchEngine.createWorkspaceScope(fToolkit
						.getCoreToolkit());
				fTitleLabel.setText(null);
			} else {
				fScope = DLTKSearchScopeFactory.getInstance()
	...
	private void disposeComponent() {
		if (fFilterActionGroup != null) {
			...
			try {
				memento.save(writer);
				fSettings.put(WORKINGS_SET_SETTINGS, writer.getBuffer()
						.toString());
			} catch (IOException e) {
