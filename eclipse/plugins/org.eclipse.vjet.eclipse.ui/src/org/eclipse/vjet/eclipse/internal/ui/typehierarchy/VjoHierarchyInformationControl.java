...
import org.eclipse.dltk.mod.internal.ui.typehierarchy.HierarchyViewerSorter;
import org.eclipse.dltk.mod.internal.ui.typehierarchy.SuperTypeHierarchyViewer.SuperTypeHierarchyContentProvider;
import org.eclipse.dltk.mod.internal.ui.typehierarchy.TraditionalHierarchyViewer.TraditionalHierarchyContentProvider;
...
public abstract class VjoHierarchyInformationControl extends
		AbstractInformationControl {

	private TypeHierarchyLifeCycle fLifeCycle;
	private VjoHierarchyLabelProvider fLabelProvider;
	...
	public VjoHierarchyInformationControl(Shell parent, int shellStyle,
			int treeStyle) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.jdt.internal.ui.text.JavaOutlineInformationControl#createTreeViewer(org.eclipse.swt.widgets.Composite,
	 *      int)
	 */
	protected TreeViewer createTreeViewer(Composite parent, int style) {
		...
		fLabelProvider = new VjoHierarchyLabelProvider(fLifeCycle,
				getPreferenceStore());
	...
	private IMethod findMethod(IMethod filterMethod, IType typeToFindIn)
		...
		if (filterOverrides) {
			return fMethodOverrideTester.findOverriddenMethodInType(
					typeToFindIn, filterMethod);
		} else {
			return fMethodOverrideTester.findOverridingMethodInType(
					typeToFindIn, filterMethod);
		}
	}
	...
	public void setInput(Object information) {
		...
		try {
			...
			default:
				DLTKUIPlugin
						.logErrorMessage("Element unsupported by the hierarchy: " + elem.getClass()); //$NON-NLS-1$
				input = null;
			}
		} catch (ModelException e) {
			DLTKUIPlugin.log(e);
		...
		try {
			fLifeCycle.ensureRefreshedTypeHierarchy(input, DLTKUIPlugin
		...
		if (topLevelObjects.length > 0
				&& contentProvider.getChildren(topLevelObjects[0]).length > 40) {
			fDoFilter = false;
		} else {
	...
	private String getHeaderLabel(IModelElement input) {
		if (input instanceof IMethod) {
			String[] args = {
					input.getParent().getElementName(),
			...
			String arg = ScriptElementLabels.getDefault().getElementLabel(
					input, ScriptElementLabels.DEFAULT_QUALIFIED);
			return Messages
					.format(
