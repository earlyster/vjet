...
public final class ProjectTemplateStore {
	...
	public ProjectTemplateStore(IProject project) {
		fInstanceStore= VjetUIPlugin.getDefault().getCodeTemplateStore();
		if (project == null) {
			fProjectStore= null;
		} else {
			final ScopedPreferenceStore projectSettings= new ScopedPreferenceStore(new ProjectScope(project), VjetUIPlugin.PLUGIN_ID);
	...
	public static boolean hasProjectSpecificTempates(IProject project) {
		String pref= new ProjectScope(project).getNode(VjetUIPlugin.PLUGIN_ID).get(KEY, null);
