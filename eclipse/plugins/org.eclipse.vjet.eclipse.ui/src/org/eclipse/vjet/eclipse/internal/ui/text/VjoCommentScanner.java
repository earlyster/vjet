 ...
 */
public class VjoCommentScanner extends AbstractScriptScanner{
	...
	private class TaskTagMatcher extends CombinedWordRule.WordMatcher {
		...
		public synchronized void addTaskTags(TodoTask todoTask) {
			addWord(todoTask.name, fToken);
		}
...
//			System.out.println(fBuffer.toString());
	...
	protected static final String TASK_TAG= VjetColorConstants.VJET_TASK_TAGS_COMMENT;
	private static final String COMPILER_TASK_TAGS= TodoTaskPreferences.TAGS;
	/**
	 * Preference key of a string preference, specifying if task tag detection is case-sensitive.
	 * @since 3.0
	 */
	private static final String COMPILER_TASK_CASE_SENSITIVE= TodoTaskPreferences.CASE_SENSITIVE;
	/**
	 * Preference value of enabled preferences.
	 * @since 3.0
	 */
	private static final String ENABLED= TodoTaskPreferences.ENABLED;
	...
	public VjoCommentScanner(IColorManager manager, IPreferenceStore store, Preferences coreStore, String defaultTokenProperty) {
		this(manager, store, coreStore, defaultTokenProperty, new String[] { defaultTokenProperty, TASK_TAG });
	}

	public VjoCommentScanner(IColorManager manager, IPreferenceStore store, Preferences coreStore, String defaultTokenProperty, String[] tokenProperties) {
		super(manager, store);
	 ...
	 * @since 3.0
	 */
	public VjoCommentScanner(IColorManager manager, IPreferenceStore store, String defaultTokenProperty) {
	 ...
	 * @since 3.0
	 */
	public VjoCommentScanner(IColorManager manager, IPreferenceStore store, String defaultTokenProperty, String[] tokenProperties) {
	...
	protected List createMatchers() {
		...
		if (tasks != null) {
			...
			List tagList=TodoTaskPreferences.getTaskTags(tasks);
			for (Object object : tagList) {
				TodoTask todoTask=(TodoTask)object;
				fTaskTagMatcher.addTaskTags(todoTask);
			}
