...
import org.eclipse.dltk.mod.internal.ui.text.ISourceVersionDependent;
import org.eclipse.dltk.mod.javascript.internal.ui.text.JavaScriptPairMatcher;
...
/**
 * Double click strategy aware of VJO identifier syntax rules.
 */
public class VjoDoubleClickSelector implements ITextDoubleClickStrategy {
	 ...
	 * @since 3.1
	 */
	private static final class AtVjoIdentifierDetector implements
		 ...
		 * 
		 * @param anchor
		 *            the offset of the double click
		 */
		...
		/*
		 * @see org.eclipse.dltk.mod.internal.ui.text.ISourceVersionDependent.setSourceVersion(String version)
		 */
		public void setSourceVersion(String version) {
			// TODO nothing, will support in further version.
		}
		...
		public IRegion getWordSelection(IDocument document, int anchor) {
			...
			try {
				...
				offset = anchor; // use to not select the previous word when
									// right behind it
				// offset= anchor - 1; // use to select the previous word when
				// right behind it
				while (offset >= min) {
					c = document.getChar(offset);
	...
	protected static final char[] BRACKETS = { '{', '}', '(', ')', '[', ']',
			'<', '>', '\"', '\"', '\'', '\'' };
	protected JavaScriptPairMatcher fPairMatcher = new JavaScriptPairMatcher(
			BRACKETS);
	protected final AtVjoIdentifierDetector fWordDetector = new AtVjoIdentifierDetector();

	public VjoDoubleClickSelector() {
		super();
	}
	...
	public void doubleClicked(ITextViewer textViewer) {
		...
		if (region != null && region.getLength() >= 2) {
			textViewer.setSelectedRange(region.getOffset() + 1, region
					.getLength() - 2);
		} else {
