...
class RenameTypeWizardInputPage extends RenameInputWizardPage {
	...
	public RenameTypeWizardInputPage(String description, String contextHelpId,
	...
	protected void addAdditionalOptions(Composite composite,
		...
		if (getSimilarElementUpdating() == null
				|| !getSimilarElementUpdating()
						.canEnableSimilarDeclarationUpdating())
			return;

		try {
			fSelectedStrategy = getRefactoringSettings().getInt(
		...
		fUpdateSimilarElements
				.setText(RefactoringMessages.RenameTypeWizardInputPage_update_similar_elements);

		final boolean updateSimilarElements = getBooleanSetting(
				RenameRefactoringWizard.TYPE_UPDATE_SIMILAR_ELEMENTS,
				getSimilarElementUpdating().getUpdateSimilarDeclarations());
		fUpdateSimilarElements.setSelection(updateSimilarElements);
		getSimilarElementUpdating().setUpdateSimilarDeclarations(
				updateSimilarElements);
		fUpdateSimilarElements.setLayoutData(new GridData(
				GridData.FILL_HORIZONTAL));
		fUpdateSimilarElements.addSelectionListener(new SelectionAdapter() {

			public void widgetSelected(SelectionEvent e) {
				getSimilarElementUpdating().setUpdateSimilarDeclarations(
						fUpdateSimilarElements.getSelection());
				fUpdateSimilarElementsButton.setEnabled(fUpdateSimilarElements
						.getSelection());
			}
		});
		...
		fUpdateSimilarElementsButton
				.setText(RefactoringMessages.RenameTypeWizardInputPage_update_similar_elements_configure);
		...
		fUpdateSimilarElementsButton
				.addSelectionListener(new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent e) {
						Assert.isTrue(false,
								"Not support update similar elements");
						// RenameTypeWizardSimilarElementsOptionsDialog dialog =
						// new RenameTypeWizardSimilarElementsOptionsDialog(
						// getShell(), fSelectedStrategy);
						// if (dialog.open() == Window.OK) {
						// fSelectedStrategy = dialog.getSelectedStrategy();
						// getSimilarElementUpdating().setMatchStrategy(
						// fSelectedStrategy);
						// }
					}
				});
	...
	public void dispose() {
		if (saveSettings())
			if (fUpdateSimilarElements != null
					&& !fUpdateSimilarElements.isDisposed()
	...
	private ISimilarDeclarationUpdating getSimilarElementUpdating() {
		return (ISimilarDeclarationUpdating) getRefactoring().getAdapter(
				ISimilarDeclarationUpdating.class);
	}
	...
	public IWizardPage getNextPage() {
		...
		if (wizard.isRenameType()) {
			final RenameVjoTypeProcessor renameTypeProcessor = wizard
