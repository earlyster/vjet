...
public class HistoryDropDownAction extends Action implements IMenuCreator {
	...
	public static class ClearHistoryAction extends Action {
		...
		public ClearHistoryAction(VJOTypeHierarchyViewPart view) {
			super(
					TypeHierarchyMessages.HistoryDropDownAction_clearhistory_label);
			fView = view;
		}
	...
	private VJOTypeHierarchyViewPart fHierarchyView;
	private Menu fMenu;

	public HistoryDropDownAction(VJOTypeHierarchyViewPart view) {
		fHierarchyView = view;
		fMenu = null;
		...
		// IJavaHelpContextIds.TYPEHIERARCHY_HISTORY_ACTION);
		setMenuCreator(this);
	}
	...
	private boolean addEntries(Menu menu, IModelElement[] elements) {
		...
		for (int i = 0; i < min; i++) {
			HistoryAction action = new HistoryAction(fHierarchyView,
					elements[i]);
			action.setChecked(elements[i].equals(fHierarchyView
					.getInputElement()));
