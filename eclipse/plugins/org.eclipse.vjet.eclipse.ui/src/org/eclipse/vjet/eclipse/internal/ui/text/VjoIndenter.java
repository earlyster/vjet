...
public final class VjoIndenter {
	 ...
	 * @since 3.1
	 */
	public VjoIndenter(IDocument document, JavaHeuristicScanner scanner,
...
//		}
	...
	public int findReferencePosition(int offset, boolean danglingElse,
		...
		int plusOffset = 0;
		if(offset+1<fDocument.getLength()){
			plusOffset = 1;
		}
		...
		case Symbols.TokenSEMICOLON:
			// check if previous token is semicolon or line new
			try {
					char prevChar = fDocument.getChar(offset-1);
					String line = fDocument.get().substring(fDocument.getLineInformationOfOffset(offset).getOffset()).trim();
					boolean isCommentRegion = line.startsWith("*") || line.startsWith("/*");
					
					if(isCommentRegion)
						return handleScopeIntroduction(offset + plusOffset);
					switch(prevChar) {
						case ';':
						...
						default:
						{
							//Add by kevin to fix bug 2537, update indent!!!  --- begin
							if (prevChar == '\n'&&offset>2
									&& fDocument.getChar(offset-2) == '\r'
									&& fDocument.getChar(offset-3) == '{') {
								pos = fPosition; 
								if (looksLikeArrayInitializerIntro() && !fPrefs.prefArrayDeepIndent)
		...
		case Symbols.TokenQUESTIONMARK:
			if (fPrefs.prefTernaryDeepAlign) {
				setFirstElementAlignment(fPosition, offset + plusOffset);
				return fPosition;
			} else {
	 ...
	 * either the token introducing the scope or - if available - the first javascript
	 * token after that.
	 * 
	 * <p>
	...
	private int handleScopeIntroduction(int bound) {
		...
		case Symbols.TokenLBRACE:
			...
			if (looksLikeArrayInitializerIntro()
				...
				// Modified by Oliver.2009-09-24. For the indentation after curly brace.
				return skipToStatementStart(true, true); 
				// set to true to match the first
//				return skipToPreviousListItemOrListStart();
			} else {
				return pos;
			}
	 ...
	 * the first Javascript token after <code>scopeIntroducerOffset</code>, but
