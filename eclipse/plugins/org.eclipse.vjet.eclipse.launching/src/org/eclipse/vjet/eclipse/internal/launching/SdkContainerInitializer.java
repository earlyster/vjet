 ...
 */
public class SdkContainerInitializer extends BuildpathContainerInitializer {
	...
	public void initialize(IPath containerPath, IScriptProject project)
		...
		if (size > 0) {
			if (containerPath.segment(0).equals(VjetPlugin.SDK_CONTAINER)) {

				IInterpreterInstall interp = resolveInterpreter(
						getNatureFromProject(project),
						getEnvironmentFromProject(project), containerPath);
				SdkBuildpathContainer container = null;
				container = new SdkBuildpathContainer(interp, containerPath);
